...
#
# xsrfh = xsrf.XSRFTokenHandler(user_func=get_user_id, secret='xsrf_secret', timeout=3600)
#
# @app.before_request
#    def before_request():
...
__all__ = [
    'XSRFTokenHandler',
    'XSRFToken',
    'XSRFTokenUserIdInvalid',
    'XSRFTokenMalformed',
    'XSRFTokenExpiredException',
    'XSRFTokenInvalid',
    'TOKEN_FORM_NAME',
    'TOKEN_HEADER_NAME'
]
...
class XSRFTokenMalformed(SniffrException):
    pass


class XSRFTokenExpiredException(SniffrException):
    pass


class XSRFTokenInvalid(SniffrException):
    pass


class XSRFTokenUserIdInvalid(SniffrException):
    pass
...
class XSRFTokenHandler:
    ...
    def send_token(self):
        ...
        def wrapper(func):
            ...
            def decorated(*args, **kw):
                ...
                response.set_cookie(
                    key=TOKEN_FORM_NAME,
                    value=session[TOKEN_FORM_NAME],
                    expires=int(time.time()) + self.timeout
                )
    ...
    def verify_token(self, token_string, request_token_string):
        ...
        if not request_token_string:
            raise XSRFTokenInvalid('Request token not valid. Please try again.')

        if token_string != request_token_string:
            raise XSRFTokenInvalid('Token not valid. Please try again.')
    ...
    def parse_xsrftoken_from_request(self):
        ...
        return request.headers.get(TOKEN_HEADER_NAME)
